...
int8_t i2c_read_bytes(uint8_t dev_address, uint8_t reg_address, uint8_t length, uint8_t *data, uint16_t timeout)
{
    ...
    {
        tx_address = dev_address;
        tx_buffer_index = 0;
        tx_buffer_length = 0;
        
        if (tx_buffer_length >= BUFFER_LENGTH) { return 0; }
        tx_buffer[tx_buffer_index] = reg_address;
        ++tx_buffer_index;
        tx_buffer_length = tx_buffer_index;
        
        uint8_t ret = twi_write(tx_address, tx_buffer, tx_buffer_length, 1, 0);
        
        uint8_t read = twi_read(address, rx_buffer, length, 1);
        rx_buffer_index = 0;
        rx_buffer_length = read;
        return read;
